...
function showIssuesList($issues, $issueKeysAddedDuringSprint, $type){
    ...
    foreach($issues as $value){
        ...
        $return['totalStoryPoints'] += $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];



//        if($value->key=='BWUI-325'){
//            echo "<pre>";
//            print_r($storyPoints);
//            die;
//        }
...
//        }
        ...
        if( $issueKeysAddedDuringSprint->$issueKey ){
            $storyPointsAddedMidSprint += $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];
        }

        if($type=='removed'){
            $storyPointsRemovedMidSprint  +=  $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];
        }

        if($type == 'completed'){
            $storyPointsCompleted += $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];
        }


        if($value->typeName == 'Bug')
            $return['bugsStoryPoints'] += $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];

        if($ftr === true){
            $return['firstTimeRightStoryPoints'] += $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];
        }

        if( isEffective($value) ){
            $return['effectiveStoryPoints'] += $storyPoints['current']?$storyPoints['current']:$storyPoints['initial'];
        }
